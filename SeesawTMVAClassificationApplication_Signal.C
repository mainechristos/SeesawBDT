...
void runSignal(std::map<std::string,int> Use, TString ifile = "./validation_Dec9.root"){
   ...
   Int_t HLTMu23TrkIsoVVLEle12CaloIdLTrackIdLIsoVLvAccept;
   Int_t HLTMu17TrkIsoVVLMu8TrkIsoVVLDZvAccept, HLTMu17TrkIsoVVLMu8TrkIsoVVLvAccept, HLTMu17TrkIsoVVLTkMu8TrkIsoVVLDZvAccept, HLTMu17TrkIsoVVLTkMu8TrkIsoVVLvAccept, HLTEle23Ele12CaloIdLTrackIdLIsoVLDZvAccept, HLTMu23TrkIsoVVLEle8CaloIdLTrackIdLIsoVLvAccept;
   ...
   //reader->AddVariable("LightLeptonPt := LightLeptonPt[0]", &LightLeptonPt );
   //reader->AddVariable("LightLeptonPt1 := LightLeptonPt[1]",&LightLeptonPt);
   //reader->AddVariable("LightLeptonPt2 := LightLeptonPt[2]",&LightLeptonBestMOSSF);
   //reader->AddVariable("LightLeptonMass := LightLeptonMass[0]",&LightLeptonMass);
   ...
   TString prefix = "2017_TMVA_test_depth_1";
   ...
   reader->BookMVA( "BDTG method",  "/cms/mchristos/ANN/Seesaw/2016/92X/weights/2017_TMVA_optimized_New_Variables.weights.xml");

   //make vectors to see passed events
   vector<double>  vecBDTG; 
   //vector<double>  vecBDTGProxy; 
   ...
   //TTree *proxyTree = (TTree*)input->Get("rootTupleProxyTree/tree");

   TTreeReader myReader(tree);
   //TTreeReader myReaderProxy(proxyTree);
   TH1I *histo= new TH1I();
   ...
   TFile *endsig = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/outFiles/SeesawTMVAClassificationApplicationVVV/Signal/QQQ/Seesaw_BDTG_Signal_1.root","RECREATE");   
   
   //TTree *tree_BDTProxy = proxyTree->CloneTree(0);
   cout<<"HI"<<endl;
   TTree *tree_BDT = tree->CloneTree(0);
   
   std::vector<double> *BDTResponse = new std::vector<double>(0);
   //std::vector<double> *BDTResponseProxy = new std::vector<double>(0);
   tree_BDT->Branch("BDT",&BDTResponse);
   //tree_BDTProxy->Branch("BDT",&BDTResponseProxy);
   ...
   while (myReader.Next()) {
      ...
      //cout<<LightLeptonBestMOSSF<<endl;
      
      if (Use["BDTG"          ]){
            double result =  reader->EvaluateMVA( "BDTG method" );
               //cout << LightLeptonPt << endl;
               BDTResponse->clear();
   ...
   /// Proxy Tree Loop ////
   ////////////////////////
   /*std::cout << "--- Processing: " << proxyTree->GetEntries() << " events" << std::endl;
   sw.Start();
   count = 0;
   
   while (myReaderProxy.Next()) {

      if (count%1000 == 0) std::cout << "--- ... Processing event: " << count << std::endl;
      
      //This will get entries to fill nonspecified branches in the cloned tree.
      proxyTree->GetEntry(count);
      
      LightLeptonBestMOSSF = inLightLeptonBestMOSSFProxy->at(0);
      if(inLightLeptonPtProxy->empty()){
         LightLeptonPt = 0;
      }else{
         LightLeptonPt = inLightLeptonPtProxy->at(0);
      }
      //cout<<"STILL WORKING :)"<<endl;
      PFMETType1 = inPFMETType1Proxy->at(0);
      LightLeptonMass = inLightLeptonMassProxy->at(0);
      LightLeptonLT = inLightLeptonLTProxy->at(0);
      if(inLightLeptonMT3Proxy->empty()){
         LightLeptonMT = 0;
      }else{
         LightLeptonMT = inLightLeptonMT3Proxy->at(0);
      }
      LightLeptonN = inLightLeptonNProxy->at(0);
      LightLeptonOnZN = inLightLeptonOnZNProxy->at(0);
      PassBadPFMuonFilter = inPassBadPFMuonFilterProxy->at(0);
      PassBadChargedCandidateFilter = inPassBadChargedCandidateFilterProxy->at(0);
      MuonN = inMuonNProxy->at(0);
      //LightLeptonTightType3D = inLightLeptonTightType3D->at(0);
      ElectronN = inElectronNProxy->at(0);
      //cout<<LightLeptonBestMOSSF<<endl;
      ...
      if (Use["BDTG"          ]){
            double result =  reader->EvaluateMVA( "BDTG method" );
               ...
               BDTResponse->clear();
               BDTResponse->push_back(result);
               if (result > -0.1153) vecBDTGProxy.push_back(count);
               tree_BDTProxy->Fill();
      }
      count++;
   }
   ...
   std::cout << "BDT: \n"<< vecBDTGProxy.size() << "\t passed events" << std::endl;
   */
   endsig->cd();
   //tree_BDTProxy->Write();
   endsig->mkdir("rootTupleTreeVeryLoose/");
   endsig->cd("rootTupleTreeVeryLoose/");
   tree_BDT->Write();
   endsig->mkdir("veryLooseDileptonEventFilter/");
   endsig->cd();
   endsig->mkdir("veryLooseDileptonEventFilter/EventCount/");
   endsig->cd("veryLooseDileptonEventFilter/EventCount/");
   histo->Write();
...
void SeesawTMVAClassificationApplication_Signal( TString myMethodList = "" ) 
{   
#ifdef __CINT__
   ...
   // --------------------------------------------------------------------------------------------------
   TString sig_name = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_QQQ_M-140_13TeV-madgraph/analysisTree.root";

   runSignal(Use, sig_name);
