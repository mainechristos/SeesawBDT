...
cout<<"LLLLL"<<endl;
   ...
   TString prefix = "2017_TMVA_new_Variables";
   ...
   cout<<"ENTRIES IN ORIGINAL TREE: "<<tree->GetEntries()<<endl;
   ...
   TFile *enddat = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/outFiles/SeesawTMVAClassificationApplicationVVV/Data/SingleMuon/QQQ/Seesaw_BDTG_Data_1.root","RECREATE"); 

   enddat->cd();
   enddat->mkdir("veryLooseDileptonEventFilter/");
   enddat->cd();
   enddat->mkdir("veryLooseDileptonEventFilter/EventCount/");
   enddat->cd("veryLooseDileptonEventFilter/EventCount/");
   ...
   enddat->mkdir("rootTupleTreeVeryLoose/");
   enddat->cd("rootTupleTreeVeryLoose/");

   TTree *tree_BDT = tree->CloneTree(0);
   cout<<tree_BDT->GetEntries()<<endl;
   cout<<tree->GetEntries()<<endl;

   std::vector<double> *BDTResponse = new std::vector<double>(0);
   tree_BDT->Branch("BDT",&BDTResponse);
   //fullTree->Branch("BDT",&BDTResponse);
   ...
   while (myReader.Next()) {
      ...
      ElectronN = inLightLeptonLT->at(0);
...
void SeesawTMVAClassificationApplication_Data_SingleMuon( TString myMethodList = "" ) 
{   
#ifdef __CINT__
   ...
   // --------------------------------------------------------------------------------------------------
   TString dat_name = "/cms/mchristos/ANN/Seesaw/2016/92X/Data/SingleMuon/QQQ/analysisTree_3L.root";

   runSignal(Use, dat_name);
